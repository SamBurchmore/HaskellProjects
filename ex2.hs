...
numGreaterThanTen [] = 0
numGreaterThanTen ((s,i):xs) = length (filter (\(s,i) -> if i>10 then True else False) ((s,i):xs))
...
sumAndLen x = foldr (\x (sum,l) -> (x+sum,l+1)) (0,0) x 
...
{- 2.2.3 Dictionary ordering
   ...
   Hint: use a 'case'. -}
   --compare the as first and then compare the bs if their equal

pair :: Comparator a -> Comparator b -> Comparator (a,b)
pair cmpA cmpB (a0,b0) (a1,b1) = case cmpA a0 a1 of { LT -> LT; GT -> GT; EQ -> cmpB b0 b1}
{- 3 MARKS -}
...
lengthOfSecond :: (Int,String) -> Int
lengthOfSecond (int,string) = length string

compareLenOfSecond :: Comparator (Int,String)
compareLenOfSecond = (on compare lengthOfSecond)

sortOnReverseSndLength :: [(Int,String)] -> [(Int,String)]
sortOnReverseSndLength x = mergeSortWith (invert compareLenOfSecond) x  
...
rectangle w h = \(x,y) -> x*x <= ((w/2)*(w/2)) && (y*y <= (h/2)*(h/2))
...
pictureAND :: Picture Bool -> Picture Bool -> Picture Bool
            ...
            --and the 2 results of the first 2 functions to get the output
pictureAND p0 p1 = \(a,b) -> (p0 (a,b)) &&  (p1 (a,b))

pictureOR :: Picture Bool -> Picture Bool -> Picture Bool
pictureOR p0 p1 = \(a,b) -> (p0 (a,b)) || (p1 (a,b))

pictureNOT :: Picture Bool -> Picture Bool
pictureNOT p0 = \(a,b) -> not (p0 (a,b))
...
colourIn picBool colour0 colour1 = \(a,b) -> if (picBool (a,b) == True) then colour0 else colour1
...
--mapPicture2 (\x y -> (x,y)) (\(x,y) -> x) (\(x,y) -> y) (10,10)
...
pictureApply :: Picture (a -> b) -> Picture a -> Picture b
               ...
               --((a -> b),(a -> b)) -> (a,a) -> (b,b)
pictureApply picF picA = \(x,y) -> picF (x,y) (picA (x,y))
...
mapPicture2' :: (a -> b -> c) -> Picture a -> Picture b -> Picture c
mapPicture2' f picA picB = pictureApply (pictureApply (everywhere f) picA) picB
...
over picFront picBack = mapPicture2 (<>) picFront picBack
...
cut mask picA = \(x,y) -> mapPicture2' (case mask (x,y) of True -> \dummyBool picA->picA; False -> mempty) mask picA (x,y)
...
transform f picA point = picA (f point) 
...
flipTopBottom picA = \(x,y)-> transform (translatePoint (0,y+y)) picA (x,y) 

flipLeftRight :: Picture a -> Picture a
flipLeftRight picA = \(x,y)-> transform (translatePoint (x+x,0)) picA (x,y) 
...
bprocess (BInput tp fp)  []        = []

bandGate :: BProcess
bandGate = BInput (BInput (BOutput True BEnd) (BOutput False BEnd)) (BInput (BOutput False BEnd) (BOutput False BEnd))

borGate :: BProcess
borGate = BInput (BInput (BOutput True BEnd) (BOutput True BEnd)) (BInput (BOutput True BEnd) (BOutput False BEnd))

bprocessToProcess :: BProcess -> Process Bool ()
bprocessToProcess BEnd = End ()
bprocessToProcess (BOutput bit p) = Output bit (bprocessToProcess p)
bprocessToProcess (BInput p0 p1) = Input (\bit -> if bit then bprocessToProcess p0 else bprocessToProcess p1)

processToBProcess :: Process Bool () -> BProcess
processToBProcess (End ()) = BEnd
processToBProcess (Output bit k) = BOutput bit (processToBProcess k)
...
sequ (End a) p2 = p2 a
sequ (Output v k) p2 = Output v (sequ k p2)
sequ (Input k) p2 = Input (\x-> sequ (k x) p2)
...
chatBot = Output "Hello! What is your name?" (Input (\inputString-> Output ("Hello "++inputString++"!") (End ())))
...
eqProcess (Output b1 k1) (Output b2 k2)
   | b1/=b2 = False
   | otherwise = eqProcess k1 k2
  -- do they output the same thing, and do they
  -- do the same thing after that?
eqProcess (Input k1) (Input k2)
   | eqProcess (k1 True) (k2 True) /= True = False
   | eqProcess (k1 False) (k2 False) /= True = False
   | otherwise = eqProcess (k1 True) (k2 True)
