...
consAll a [[]] = [[a]]
consAll a [b] = [a:b]
consAll a (b:bs) = [a:b]++(consAll a bs)
...
splitOnHelper s y [] = (reverse y):[]
splitOnHelper s y (x:xs)
   | s /= x = splitOnHelper s (x:y) xs
   | otherwise = (reverse y):splitOnHelper s [] xs
...
words :: String -> [String]
words [] = []
words (x:xs) = removeEmpty (splitOn ' ' (x:xs))
...
layOutLines w 0 line (x:xs) = layOutLines w (plusWord 0 x) (x:line) xs
layOutLines w col line (x:xs)
   | (w+1) >= (plusWord col x) = layOutLines w (plusWord col x) (x:line) xs
   | otherwise = (reverse line):layOutLines w 0 [] (x:xs)
...
delete (Within [] point (a:after)) = Within [] a after
...
overwrite x (AtEnd before) = Within before x []
overwrite x (Within before point after) = Within before x after
...
currentLine (LineCursor above line below) = line 

updateLine :: LineCursor -> Cursor -> LineCursor
updateLine (LineCursor above line below) x = LineCursor above x below

moveUp :: LineCursor -> LineCursor
moveUp (LineCursor [] line below) = (LineCursor [] line below)
moveUp (LineCursor (a:above) line below) = LineCursor above (toCursor(a)) ((fromCursor(line)):below)

moveDown :: LineCursor -> LineCursor
moveDown (LineCursor above line []) = LineCursor above line []
moveDown (LineCursor above line (b:below)) = LineCursor ((fromCursor(line)):above) (toCursor(b)) below
{- 6 MARKS -}
...
insertKey key value [] = [(key,value)]
insertKey key value ((k,v):kvs)
   | key==k = ((k,value):kvs)
   | key<k = ((key,value):((k,v):kvs))
   | otherwise = (k,v):(insertKey key value kvs)
...
getKey ([]) c = case c of {(Value []) -> Nothing; c -> Just c}
getKey (x:xs) (Value v) = Nothing
getKey (x:xs) (Store list) = getKey xs (case (lookupKey x list) of { Nothing ->(Value []); Just y -> y})
...
getValue k c = case (getKey k c) of {Nothing->KeyMissing; Just (Value x) -> Ok x; Just (Store x) -> KeyNotAValue} 
...
merge (Value x) (Value y) = (Value y)
merge (Store x) (Value y) = (Value y)
merge (Value x) (Store y) = (Store y)
merge (Store []) (Store []) = emptyConfig
merge (Store (x:xs)) (Store []) = (Store (x:xs))
merge (Store []) (Store (y:ys)) = (Store (y:ys))
merge (Store (x:xs)) (Store (y:ys)) = (Store (mergeKVs (x:xs) (y:ys)))


mergeKVs :: [(String,Config)] -> [(String,Config)] -> [(String,Config)]
mergeKVs [] ys = ys
mergeKVs xs [] = xs
mergeKVs ((xK,xV):xs) ((yK,yV):ys)
   | xK < yK = (xK,xV):mergeKVs xs ((yK,yV):ys)
   | xK > yK = (yK,yV):mergeKVs ((xK,xV):xs) ys
   | otherwise = (yK,yV):(case (merge (Store xs) (Store ys)) of { (Store x) -> x;})
...
update k v (Store []) = set k v
update k v (Store c) = merge (Store c) (set k v)
update k v (Value s) = merge (set [] s) (set k v)
---update [] v (Store [c]) = merge (Store [c]) (set [] v)
{- 1 MARK -}
...
expects (b:bs) 
   |b==True = Input (expects bs) (expectsHelper bs)
   |otherwise = Input (expectsHelper bs) (expects bs)

expectsHelper :: [Bool] -> Process
expectsHelper [] = Output False End
expectsHelper (b:bs)
   |b==True = Input (expectsHelper bs) (Output False End)
   |otherwise = Input (Output False End) (expectsHelper bs)   
...
pipe (Output b p) (Input t f) = pipe p (if b==True then t else f) -- what goes here?
...
pipe (Input t1 f1) p2 = Input (pipe t1 p2) (pipe f1 p2) -- what happens in each case?
